...
const EachSkillDesc: React.FC<SkillTypeContent> = ({ title, description, skills, iconComponent}) => {
  return (
    <Grid className="skill-desc" item xs={12} sm={6} md={6} lg={6} style={{ backgroundColor : '#33CCCC', maxWidth: '40%', margin : '25px', padding: '20px', borderRadius: '5px' }}>
      <Typography variant="h5" gutterBottom style={{ textAlign : 'center' }}>{title}</Typography>
      ...
      <Table style={{ border: 'none' }}>
        <TableHead>
          <TableRow>
            ...
            <TableCell>スキルレベル</TableCell>
          </TableRow>
        </TableHead>
        <TableBody style={{border: 'none'}}>
            {skills.map((skill, index) => (
              <TableRow key={index} style={{ border: 'none' }}>
                <TableCell style={{border: "none"}}>{skill.technology}</TableCell>
                <TableCell style={{border: 'none'}}>{skill.yearsOfExperience}</TableCell>
                <TableCell style={{border: 'none'}}>{'★'.repeat(skill.rating)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
      </Table>
...
const Skill: React.FC = () => {
  return (
    <div id="skill" className="my-profile" style={{ display: 'block', marginTop: '100px'}}>
        ...
        <EachSkillDesc
          ...
          <EachSkillDesc
            ...
            skills={[
              ...
              { technology: 'オンプレミス', yearsOfExperience: '3ヶ月', rating: 2 },
            ]}
            iconComponent={<SkillIcon />}
          />
          <EachSkillDesc
